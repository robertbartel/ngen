...
namespace realization {
  namespace config {
    ...
    //!
    //! The hydrofabric names a divide's identifier `divide_id`, and an auxiliary table keyed to
    //! divides follows it.
    ...
    namespace {
      ...
      inline std::string aux_scalar_string(const boost::property_tree::ptree& node, const std::string& key,
       ...
       */
      inline ngen::geopackage::AttributeJoinSpec
      parse_auxiliary_attribute_entry(const boost::property_tree::ptree& entry, const std::string& context)
      {
          if (entry.empty()) {
              throw std::runtime_error(context + ": entry must be an object declaring at least a 'table' name.");
          }

          ngen::geopackage::AttributeJoinSpec parsed;
          parsed.key_column = DEFAULT_AUX_ATTRIBUTES_KEY_COLUMN;
          ...
          parsed.table = aux_scalar_string(*table_node, "table", context);

          if (const auto alias_node = entry.get_child_optional("alias")) {
              parsed.alias = aux_scalar_string(*alias_node, "alias", context);
          }
          if (const auto file_node = entry.get_child_optional("file")) {
              parsed.file = aux_scalar_string(*file_node, "file", context);
          }
          if (const auto key_column_node = entry.get_child_optional("key_column")) {
              parsed.key_column = aux_scalar_string(*key_column_node, "key_column", context);
          }
     ...
     * @ref ngen::geopackage::AttributeJoinSpec::prefix) must be unique across entries, since a repeated
     * prefix would let two tables' columns collide in the feature property map.
     *
     * @throw std::runtime_error if the key is not a list, an entry is malformed, or two entries resolve
     *        to the same prefix.
     */
    inline std::vector<ngen::geopackage::AttributeJoinSpec>
    parse_auxiliary_attributes(const boost::property_tree::ptree& realization_tree)
    {
        ...
        std::vector<ngen::geopackage::AttributeJoinSpec> entries;
        ...
        for (const auto& child : *node) {
            ...
            ngen::geopackage::AttributeJoinSpec entry =
                parse_auxiliary_attribute_entry(child.second, context);
