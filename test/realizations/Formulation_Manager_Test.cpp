...
class Formulation_Manager_Test : public ::testing::Test {
    ...
    {
        const std::string path = ngen::geopackage::fixtures::write_example_aux();

        geojson::GeoJSON collection = ngen::geopackage::make_reader(path).read("test", {});

        ngen::geopackage::AttributeJoinSpec one;
        one.table = "aux_params_one";
        one.alias = "one";
        one.key_column = "divide_id";

        ngen::geopackage::AttributeJoinSpec two;
        two.table = "aux_params_two";
        two.alias = "two";
        two.key_column = "catchment_id";

        ngen::geopackage::join_attributes(*collection, path, one);
        ngen::geopackage::join_attributes(*collection, path, two);
        return collection;
    }
#endif
 ...
 *
 * Both `from` values name a `real_value` column -- one per attribute table of the fixture written
 * by write_example_aux() -- so only the joined-on prefix tells them apart. Every divide shares one
 * forcing file, since the fixture's ids have none of their own.
 */
