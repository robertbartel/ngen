...
{
  protected:
    //! Built once for the suite, since no test here modifies it.
    static void SetUpTestSuite()
    {
        path_aux = ngen::geopackage::fixtures::write_example_aux();
    }
    ...
    std::string path;
    std::string path2;
    static std::string path_aux;
};

std::string GeoPackage_Test::path_aux;

TEST_F(GeoPackage_Test, geopackage_read_test)
{
    const auto gpkg = ngen::geopackage::make_reader(this->path).read("test", {});
...
{
    const auto gpkg = ngen::geopackage::make_reader(this->path).read("test", { "First" });
    EXPECT_NE(gpkg->find("First"), -1);
...
{
    const auto gpkg = ngen::geopackage::make_reader(this->path2).read("example_3857", {});
    EXPECT_NE(gpkg->find("First"), -1);
    EXPECT_NE(gpkg->find("Second"), -1);
    const auto bbox = gpkg->get_bounding_box();
    EXPECT_EQ(bbox.size(), 4);
...
// The fixture is a copy of example.gpkg carrying four auxiliary attribute tables; see
// write_example_aux() for what they hold and why.
TEST_F(GeoPackage_Test, geopackage_aux_fixture_test)
{
    const auto gpkg = ngen::geopackage::make_reader(this->path_aux).read("test", {});
    EXPECT_EQ(2, gpkg->get_size());
    EXPECT_NE(gpkg->find("First"), -1);
    EXPECT_NE(gpkg->find("Second"), -1);
